...
#include "../window/dialog_delegate.h"
...
namespace view
{
    ...
    void DialogClientView::ShowDialogButtons()
    {
        ...
        if(buttons&MessageBoxFlags::DIALOGBUTTON_OK && !ok_button_)
        {
            ...
            if(label.empty())
            {
                label = UTF16ToWide(GetStringUTF16(IDS_APP_OK));
            }
        ...
        if(buttons&MessageBoxFlags::DIALOGBUTTON_CANCEL && !cancel_button_)
        {
            ...
            if(label.empty())
            {
                ...
                {
                    label = UTF16ToWide(GetStringUTF16(IDS_APP_CANCEL));
                }
                else
                {
                    label = UTF16ToWide(GetStringUTF16(IDS_APP_CLOSE));
                }
            }
            bool is_default_button =
