...
namespace view
{
    ...
    bool RootView::ProcessKeyEvent(const KeyEvent& event)
    {
        ...
        // keyboard.
        if(v && v->IsEnabled() && ((event.key_code()==VKEY_APPS) ||
            (event.key_code()==VKEY_F10 && event.IsShiftDown())))
        {
        ...
        {
            consumed = (event.type() == ET_KEY_PRESSED) ?
    ...
    void RootView::GetAccessibleState(AccessibleViewState* state)
    {
        state->role = AccessibilityTypes::ROLE_APPLICATION;
    }
