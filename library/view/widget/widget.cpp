...
namespace view
{
    ...
    void Widget::Init(HWND parent, const gfx::Rect& bounds)
    {
        GetRootView();
    ...
    bool Widget::GetAccelerator(int cmd_id, MenuAccelerator* accelerator)
    {
        return false;
    }
    ...
    void Widget::RunShellDrag(View* view, const OSExchangeData& data, int operation)
    {
        dragged_view_ = view;
    ...
    bool Widget::EnsureCompositor()
    {
        ...
        HWND widget = native_widget_->GetAcceleratedWidget();
        if(widget != NULL)
        {
            compositor_ = gfx::Compositor::Create(widget);
        }
