...
namespace
{
    ...
    void GetCompositionUnderlines(HIMC imm_context,
        int target_start,
        int target_end,
        view::CompositionUnderlines* underlines)
    {
        ...
        if(clause_length)
        {
            ...
            if(clause_data.get())
            {
                ...
                {
                    view::CompositionUnderline underline;
...
namespace view
{
        ...
        if(lparam & type)
        {
            ...
            if(string_size > 0)
            {
                ...
                if(string_data)
                {
                    ...
                    ::ImmGetCompositionString(imm_context, type, string_data,
                        string_size);
                    ret = true;
                }
            }
    ...
    std::string ImeInput::GetInputLanguageName() const
    {
        ...
        // Get language id.
        int length = ::GetLocaleInfo(locale_id, LOCALE_SISO639LANGNAME,
            &buffer[0], arraysize(buffer));
        if(length <= 1)
        {
            return std::string();
        }
        ...
        // Get region id.
        length = ::GetLocaleInfo(locale_id, LOCALE_SISO3166CTRYNAME,
            &buffer[0], arraysize(buffer));
        if(length <= 1)
        {
            return language;
    ...
    base::TextDirection ImeInput::GetTextDirection() const
    {
        return IsRTLPrimaryLangID(PRIMARYLANGID(input_language_id_)) ?
            base::RIGHT_TO_LEFT : base::LEFT_TO_RIGHT;
    }
    ...
    bool ImeInput::IsCtrlShiftPressed(base::TextDirection* direction)
    {
        ...
        if(keystate[VK_RSHIFT] & kKeyDownMask)
        {
            ...
            *direction = base::RIGHT_TO_LEFT;
        }
        else if(keystate[VK_LSHIFT] & kKeyDownMask)
        {
            keystate[VK_LSHIFT] = 0;
            *direction = base::LEFT_TO_RIGHT;
        }
        else
        {
            return false;
