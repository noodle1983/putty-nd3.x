...
namespace view
{
    ...
    namespace
    {
        ...
        {
        public:
            GradientPainter(bool horizontal, const SkColor& top,
                const SkColor& bottom) : horizontal_(horizontal)
            {
                colors_[0] = top;
                colors_[1] = bottom;
            }
            ...
            void Paint(int w, int h, gfx::Canvas* canvas)
            {
                ...
                SkShader* s = SkGradientShader::CreateLinear(p, colors_,
                    NULL, 2, SkShader::kClamp_TileMode, NULL);
