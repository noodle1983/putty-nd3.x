...
namespace view
{
    ...
    HWND TreeView::CreateNativeControl(HWND parent_container)
    {
        ...
        original_handler_ = SetWindowProc(tree_view_, &TreeWndProc);
        AdjustUIFontForWindow(tree_view_);
        ...
        case TVN_KEYDOWN:
            if(controller_)
            {
                ...
                controller_->OnTreeViewKeyDown(
                    KeyboardCodeForWindowsKeyCode(key_down_message->wVKey));
            }
            break;
    ...
    bool TreeView::OnKeyDown(KeyboardCode virtual_key_code)
    {
        ...
        else if(virtual_key_code==VKEY_RETURN && !process_enter_)
        {
    ...
    void TreeView::CreateItem(HTREEITEM parent_item,
        HTREEITEM after, TreeModelNode* node)
    {
        DCHECK(node);
