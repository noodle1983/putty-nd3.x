...
namespace view
{
    ...
    // static
    const char CustomButton::kViewClassName[] = "view/CustomButton";
    ...
    void CustomButton::SetHotTracked(bool flag)
    {
        ...
        if(flag && GetWidget())
        {
            GetWidget()->NotifyAccessibilityEvent(
                this, AccessibilityTypes::EVENT_FOCUS, true);
        }
    }
    ...
    bool CustomButton::OnKeyPressed(const KeyEvent& event)
    {
        ...
        if(event.key_code() == VKEY_SPACE)
        {
            SetState(BS_PUSHED);
        }
        else if(event.key_code() == VKEY_RETURN)
        {
            SetState(BS_NORMAL);
            NotifyClick(event);
        }
    ...
    {
        if((state_==BS_DISABLED) || (event.key_code()!=VKEY_SPACE))
        {
            return false;
        }
    ...
    bool CustomButton::AcceleratorPressed(const Accelerator& accelerator)
    {
        ...
        SetState(BS_NORMAL);
        KeyEvent key_event(ET_KEY_RELEASED, accelerator.GetKeyCode(),
    ...
    void CustomButton::GetAccessibleState(AccessibleViewState* state)
    {
        Button::GetAccessibleState(state);
        switch(state_)
        {
        case BS_HOT:
            state->state = AccessibilityTypes::STATE_HOTTRACKED;
            break;
        case BS_PUSHED:
            state->state = AccessibilityTypes::STATE_PRESSED;
            break;
        case BS_DISABLED:
            state->state = AccessibilityTypes::STATE_UNAVAILABLE;
            break;
        case BS_NORMAL:
        case BS_COUNT:
    ...
    CustomButton::CustomButton(ButtonListener* listener)
        ...
        is_throbbing_(false),
        triggerable_event_flags_(EF_LEFT_BUTTON_DOWN),
    ...
    void CustomButton::ViewHierarchyChanged(bool is_add, View* parent, View* child)
    {
