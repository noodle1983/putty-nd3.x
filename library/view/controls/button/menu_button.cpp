...
#include "../menu/view_menu_delegate.h"
...
namespace view
{
    ...
    void MenuButton::OnMouseReleased(const MouseEvent& event)
    {
        ...
        if(GetDragOperations(event.location())!=DragDropTypes::DRAG_NONE &&
    ...
    void MenuButton::GetAccessibleState(AccessibleViewState* state)
    {
        CustomButton::GetAccessibleState(state);
        state->role = AccessibilityTypes::ROLE_BUTTONMENU;
        state->default_action = GetStringUTF16(IDS_APP_ACCACTION_PRESS);
