...
namespace view
{
    ...
    void TextButton::PaintButton(gfx::Canvas* canvas, PaintButtonMode mode)
    {
        ...
        {
            icon_x = std::max(0, (available_width - content_width) / 2) +
                insets.left();
        }
        int text_x = icon_x;
        if(icon.width() > 0)
    ...
    void TextButton::UpdateTextSize()
    {
        ...
        gfx::CanvasSkia::SizeStringInt(text_, font_, &width, &height,
    ...
    gfx::Size TextButton::GetPreferredSize()
    {
        ...
        gfx::Size prefsize(max_text_size_.width()+icon_.width()+insets.width(),
            std::max(max_text_size_.height(), icon_.height())+insets.height());
