...
namespace view
{
    ...
    HWND NativeTableWin::GetTestingHandle() const
    {
        return native_view();
    }
    ...
    void NativeTableWin::CreateNativeControl()
    {
        ...
        original_handler_ = ui::SetWindowProc(hwnd, &NativeTableWin::TableWndProc);
                ...
                if(table_->type() == ICON_AND_TEXT)
                {
                    ...
                    if(!image.isNull())
                    {
                        ...
                        if(ListView_GetItemRect(native_view(), view_index, &icon_rect,
                            LVIR_ICON) && GetClientRect(native_view(), &client_rect))
                        {
                            RECT intersection;
                            ...
                            if(IntersectRect(&intersection, &icon_rect, &client_rect))
                            {
                                ...
                                int selected = ListView_GetItemState(native_view(),
                                ...
                                RECT to_draw = { intersection.left-icon_rect.left,
                                    intersection.top-icon_rect.top, 0, 0 };
                        ...
                        else if(!table->single_selection() && w_param&MK_SHIFT)
                        {
                            ...
                            if(mark_view_index != -1)
                            {
                                ...
                                for(int i=std::min(view_index, mark_view_index),
                                    max_i=std::max(view_index, mark_view_index);
                                    i<=max_i; ++i)
                                {
