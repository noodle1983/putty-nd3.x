...
namespace view
{
    ...
    namespace
    {
        ...
        bool TitleMatchesMnemonic(MenuItemView* menu, wchar_t key)
        {
            ...
            std::wstring lower_title = UTF16ToWide(
                ToLower(WideToUTF16(menu->GetTitle())));
        ...
        const MouseEvent& event)
    {
        ...
        if(!part.is_scroll() && part.menu && !(part.menu->HasSubmenu() &&
            (event.flags()==EF_LEFT_BUTTON_DOWN)))
        {
        ...
        int selection_types)
    {
            ...
            menu_item->GetType()!=MenuItemView::SUBMENU))
        {
            menu_item->GetWidget()->NotifyAccessibilityEvent(
                menu_item, AccessibilityTypes::EVENT_FOCUS, true);
        }
    }
    ...
    bool MenuController::SendAcceleratorToHotTrackedView()
    {
        ...
        Accelerator accelerator(VKEY_RETURN, false, false, false);
        ...
        const MouseEvent& event)
    {
        ...
        HWND window_under_mouse = Screen::GetWindowAtCursorScreenPoint();
        ...
        *is_leading = prefer_leading;
        ...
        if(!item->GetParentMenuItem())
        {
                ...
                y+pref.height()>state_.monitor_bounds.bottom())
            {
                ...
                if(y < (state_.monitor_bounds.y()+state_.monitor_bounds.height()/2))
                {
    ...
    bool MenuController::SelectByChar(wchar_t character)
    {
        ...
        wchar_t key = UTF16ToWide(ToLower(WideToUTF16(char_array)))[0];
        MenuItemView* item = pending_state_.item;
        if(!item->HasSubmenu() || !item->GetSubmenu()->IsShowing())
        {
            item = item->GetParentMenuItem();
        ...
        const MouseEvent& event, View* target_menu)
    {
        ...
        if(target != active_mouse_view_)
        {
            ...
            if(active_mouse_view_)
            {
                ...
                MouseEvent mouse_pressed_event(ET_MOUSE_PRESSED,
        ...
        if(active_mouse_view_)
        {
            gfx::Point target_point(target_menu_loc);
            View::ConvertPointToView(target_menu, active_mouse_view_, &target_point);
            MouseEvent mouse_dragged_event(ET_MOUSE_DRAGGED,
                target_point.x(), target_point.y(), event.flags());
