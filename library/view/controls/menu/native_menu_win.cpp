...
namespace view
{
    ...
    public:
        ...
        {
            RegisterClass();
            hwnd_ = CreateWindowEx(GetExtendedStyles(), kWindowClassName,
                L"", 0, 0, 0, 0, 0, HWND_MESSAGE, NULL, NULL, NULL);
            SetWindowUserData(hwnd_, this);
        }
    ...
    private:
        ...
        void RegisterClass()
        {
            ...
            wcex.lpfnWndProc = base::WrappedWindowProc<&MenuHostWindowProc>;
        ...
        void OnMenuCommand(int position, HMENU menu)
        {
            ...
            MenuModel* model = intergoat->model_;
        ...
        {
            MenuHostWindow* host =
                reinterpret_cast<MenuHostWindow*>(GetWindowUserData(window));
    ...
    NativeMenuWin::NativeMenuWin(MenuModel* model, HWND system_menu_for)
        : model_(model),
        menu_(NULL),
        owner_draw_(NeedOverrideDefaultUIFont(NULL, NULL) &&
        !system_menu_for),
    ...
    void NativeMenuWin::Rebuild()
    {
            ...
            menu_index<first_item_index_+model_->GetItemCount(); ++menu_index)
        {
            ...
            if(model_->GetTypeAt(model_index) == MenuModel::TYPE_SEPARATOR)
            {
    ...
    void NativeMenuWin::AddMenuItemAt(int menu_index, int model_index)
    {
        ...
        MenuModel::ItemType type = model_->GetTypeAt(model_index);
        if(type == MenuModel::TYPE_SUBMENU)
        {
            item_data->submenu.reset(
                new Menu2(model_->GetSubmenuModelAt(model_index)));
            mii.fMask |= MIIM_SUBMENU;
            mii.hSubMenu = item_data->submenu->GetNativeMenu();
        }
        else
        {
            if(type == MenuModel::TYPE_RADIO)
            {
        ...
        const std::wstring& label)
    {
        ...
        if(type != MenuModel::TYPE_SUBMENU)
        {
            ...
            Accelerator accelerator(VKEY_UNKNOWN, false, false, false);
