...
#include "../view/custom_frame_view.h"
#include "../view/native_frame_view.h"
...
namespace view
{
    ...
    gfx::Font WindowWin::GetWindowTitleFont()
    {
        ...
        AdjustUIFont(&(ncm.lfCaptionFont));
    ...
    void WindowWin::Init(HWND parent, const gfx::Rect& bounds)
    {
        ...
        GetMonitorAndRects(bounds.ToRECT(), &last_monitor_,
            &last_monitor_rect_, &last_work_area_);

        WidgetWin::Init(parent, bounds);

        SetWindowUserData(GetNativeView(), this);
    ...
    LRESULT WindowWin::OnNCUAHDrawCaption(UINT msg, WPARAM w_param, LPARAM l_param)
    {
    ...
    LRESULT WindowWin::OnNCUAHDrawFrame(UINT msg, WPARAM w_param, LPARAM l_param)
    {
        // See comment in widget_win.h at the definition of WM_NCUAHDRAWCAPTION for
        // an explanation about why we need to handle this message.
        SetMsgHandled(!delegate_->IsUsingNativeFrame());
    ...
    void WindowWin::OnWindowPosChanging(WINDOWPOS* window_pos)
    {
        ...
        else if(!GetParent())
        {
            ...
            if(GetWindowRect(&window_rect) && GetMonitorAndRects(window_rect, &monitor,
                &monitor_rect, &work_area))
            {
    ...
    void WindowWin::CenterWindow(const gfx::Size& size)
    {
        ...
        CenterAndSizeWindow(parent, GetNativeView(), size, false);
    }
    ...
    void WindowWin::SetUseDragFrame(bool use_drag_frame)
    {
        if(use_drag_frame)
        {
            ...
            // Remove the captions style so the window doesn't have window controls for a
