...
namespace view
{
    ...
    void Label::SetHorizontalAlignment(Alignment alignment)
    {
        ...
        if(base::IsRTL() && (rtl_alignment_mode_==USE_UI_ALIGNMENT) &&
    ...
    void Label::GetAccessibleState(AccessibleViewState* state)
    {
        state->role = AccessibilityTypes::ROLE_STATICTEXT;
        ...
        int* flags) const
    {
        ...
        if(elide_in_middle_)
        {
            *paint_text = ElideText(text_, font_, GetAvailableRect().width(), true);
        }
        else
        {
            *paint_text = text_;
