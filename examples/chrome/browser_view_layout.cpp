...
namespace
{
    ...
    bool ConvertedHitTest(view::View* src, view::View* dst, gfx::Point* point)
    {
        DCHECK(src);
        DCHECK(dst);
        DCHECK(point);
        view::View::ConvertPointToView(src, dst, point);
    ...
    const gfx::Point& point)
{
    ...
    view::View::ConvertPointToView(browser_view_, tabstrip_, &tabstrip_point);
...
int BrowserViewLayout::NonClientHitTest(const gfx::Point& point)
{
    ...
    view::View* parent = browser_view_->parent();

    gfx::Point point_in_browser_view_coords(point);
    view::View::ConvertPointToView(
    ...
    if(browser_view_->IsTabStripVisible())
    {
        ...
        if(!browser_view_->IsMaximized() &&
            (point_in_browser_view_coords.y() < (tabstrip_->y()+kTabShadowSize)))
        {
...
void BrowserViewLayout::ViewAdded(view::View* host, view::View* view)
{
    ...
    case VIEW_ID_CONTENTS_SPLIT:
        {
            contents_split_ = static_cast<view::SingleSplitView*>(view);
...
int BrowserViewLayout::LayoutTabStripRegion()
{
    ...
    view::View::ConvertPointToView(browser_view_->parent(), browser_view_,
        &tabstrip_origin);
...
int BrowserViewLayout::LayoutBookmarkBar(int top)
{
    ...
    {
        y -= view::NonClientFrameView::kClientEdgeThickness +
    ...
    const gfx::Point& future_parent_offset)
{
    ...
    // coordinates.
    view::View::ConvertPointToView(browser_view_, source->parent(),
...
void BrowserViewLayout::LayoutTabContents(int top, int bottom)
{
    ...
    //view::SingleSplitView* sidebar_split = browser_view_->sidebar_split_;
    //if(sidebar_split)
    //{
...
int BrowserViewLayout::GetTopMarginForActiveContent()
{
    ...
    //{
    //    view::View* sidebar_split = contents_split_->child_at(0);
