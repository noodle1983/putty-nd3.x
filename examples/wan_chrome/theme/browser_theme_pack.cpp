...
bool BrowserThemePack::WriteToDisk(FilePath path) const
{
    ...
    resources[kTintsID] = base::StringPiece(
        ...
        sizeof(TintEntry[kTintArraySize]));
...
bool BrowserThemePack::GetTint(int id, gfx::HSL* hsl) const
{
    if(tints_)
    {
...
void BrowserThemePack::BuildTintsFromJSON(DictionaryValue* tints_value)
{
    ...
    std::map<int, gfx::HSL> temp_tints;
    ...
    int count = 0;
    for(std::map<int, gfx::HSL>::const_iterator it=temp_tints.begin();
        it!=temp_tints.end()&&count<kTintArraySize; ++it,++count)
    {
...
void BrowserThemePack::ReadColorsFromJSON(DictionaryValue* colors_value,
                                          std::map<int, SkColor>* temp_colors)
{
            ...
            ((color_list->GetSize()==3) || (color_list->GetSize()==4)))
        {
                ...
                color_list->GetInteger(2, &b))
            {
                if(color_list->GetSize() == 4)
                {
                    ...
                    {
                        color = SkColorSetARGB(static_cast<int>(alpha*255),
                            r, g, b);
                    }
...
void BrowserThemePack::GenerateMissingColors(std::map<int, SkColor>* colors)
{
                                        ...
                                        ImageCache* raw_bitmaps)
{
        ...
        it!=file_paths.end(); ++it)
    {
        ...
        else if(raw_data.get() && raw_data->size())
        {
            // Decode the PNG.
            SkBitmap bitmap;
            if(gfx::PNGCodec::Decode(raw_data->front(), raw_data->size(), &bitmap))
            {
...
void BrowserThemePack::GenerateTintedButtons(const gfx::HSL& button_tint,
...
{
    DCHECK(BrowserThread::CurrentlyOn(BrowserThread::FILE));
    for(ImageCache::const_iterator it=images.begin(); it!=images.end(); ++it)
    {
                                        ...
                                        ImageCache* destination) const
{
    ...
    for(ImageCache::const_iterator it=source.begin(); it!=source.end(); ++it)
    {
...
{
    DCHECK(BrowserThread::CurrentlyOn(BrowserThread::FILE));
    for(RawImages::const_iterator it=images.begin(); it!=images.end(); ++it)
    {
...
gfx::HSL BrowserThemePack::GetTintInternal(int id) const
{
    if(tints_)
    {
        for(int i=0; i<kTintArraySize; ++i)
