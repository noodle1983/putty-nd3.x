...
void InfoBarContainer::ChangeTabContents(TabContents* contents)
{
    ...
    if(tab_contents_)
    {
        ...
        for(size_t i=0; i<tab_contents_->infobar_count(); ++i)
        {
            ...
            AddInfoBar(tab_contents_->GetInfoBarDelegateAt(i)->CreateInfoBar(),
                false, NO_CALLBACK);
        }
    }
...
void InfoBarContainer::GetAccessibleState(AccessibleViewState* state)
{
    state->role = AccessibilityTypes::ROLE_GROUPING;
    state->name = view::GetStringUTF16(IDS_ACCNAME_INFOBAR_CONTAINER);
}
