...
BrowserFrame* BrowserFrame::Create(BrowserView* browser_view, Profile* profile)
{
...
view::Window* BrowserFrameWin::GetWindow()
{
    return this;
}
...
view::View* BrowserFrameWin::GetFrameView() const
{
...
bool BrowserFrameWin::GetAccelerator(int cmd_id, MenuAccelerator* accelerator)
{
...
void BrowserFrameWin::OnInitMenuPopup(HMENU menu, UINT position, BOOL is_system_menu)
{
    browser_view_->PrepareToRunSystemMenu(menu);
}

LRESULT BrowserFrameWin::OnMouseActivate(HWND window, UINT hittest_code, UINT message)
{
    return browser_view_->ActivateAppModalDialog() ? MA_NOACTIVATEANDEAT
        : MA_ACTIVATE;
}

void BrowserFrameWin::OnMove(const gfx::Point& point)
{
...
LRESULT BrowserFrameWin::OnNCHitTest(const gfx::Point& pt)
{
