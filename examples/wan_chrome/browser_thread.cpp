...
public:
    ...
    {
        return BrowserThread::PostTask(id_, task);
    }

    virtual bool PostDelayedTask(Task* task, int64 delay_ms)
    {
        return BrowserThread::PostDelayedTask(id_, task, delay_ms);
    }

    virtual bool PostNonNestableTask(Task* task)
    {
        return BrowserThread::PostNonNestableTask(id_, task);
    }

    virtual bool PostNonNestableDelayedTask(Task* task, int64 delay_ms)
    {
        return BrowserThread::PostNonNestableDelayedTask(id_, task, delay_ms);
    }
...
// static
bool BrowserThread::PostTask(ID identifier, Task* task)
{
    return PostTaskHelper(identifier, task, 0, true);
}

// static
bool BrowserThread::PostDelayedTask(ID identifier, Task* task, int64 delay_ms)
{
    return PostTaskHelper(identifier, task, delay_ms, true);
}

// static
bool BrowserThread::PostNonNestableTask(ID identifier, Task* task)
{
    return PostTaskHelper(identifier, task, 0, false);
}
...
bool BrowserThread::PostNonNestableDelayedTask(ID identifier,
                                               ...
                                               int64 delay_ms)
{
    return PostTaskHelper(identifier, task, delay_ms, false);
}
...
// static
bool BrowserThread::PostTaskHelper(ID identifier,
                                   Task* task,
                                   int64 delay_ms,
                                   bool nestable)
{
